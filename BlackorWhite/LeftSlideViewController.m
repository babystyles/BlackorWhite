...
- (instancetype)initWithLeftView:(UIViewController *)leftVC
    ...
    if (self) {
        ...
        [self.sideslipTapGes setEnabled:NO];
...
- (void)openLeftView{
    ...
    [self.sideslipTapGes setEnabled:YES];
}
...
- (void)panGestureAction:(UIPanGestureRecognizer *)panAction{
    ...
    // 根据视图位置判断是左滑还是右滑
    if (needMoveWithTap && (panAction.view.frame.origin.x >= 0) && (panAction.view.frame.origin.x <= (WIDTH - 60))) {
